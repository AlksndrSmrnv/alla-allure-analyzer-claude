...
| Поле | Тип | Обязат. | Описание |
|---|---|---|---|
| `summary_text` | string | да | 2–4 абзаца. |
| `key_findings` | array[string] | нет | Буллеты главных наблюдений. |
| `priority_actions` | array[string] | нет | 1–3 конкретных действия. |
| `unanalyzed_tail` | object | нет | Только для режима >30. |
...
| `analysis_text` | string | да | 1..8000 символов. Для `unanalyzed` — короткая пометка вида `tail (size=4)`. |
| `kb_alignment` | object | да | См. ниже. |
| `recommendations` | array[string] | нет | Буллеты конкретных действий. |
...
* `recommendations` — конкретные действия (кого пинать / что чинить /
  где искать).
