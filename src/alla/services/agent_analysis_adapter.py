...
Сохранённый серверный API принимает агентский JSON, проверяет его
функцией ``validate_agent_payload`` этого модуля и конвертирует
в существующие модели (контракт автономного Qwen-скилла отдельный):
