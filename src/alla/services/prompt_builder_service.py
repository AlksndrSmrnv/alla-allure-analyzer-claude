...
def build_cluster_analysis_prompt(
    ...
    *,
    ...
    parts.append("═══════════════════════════════════════")
    parts.append("")
    parts.append(_build_cluster_instruction(kb_matches))
...
_SYSTEM_PROMPT_CLUSTER = "\n".join(
    [
        ...
        "1. Сообщение ошибки / стек-трейс / лог — это первичные факты. "
        "На них строится анализ.",
        ...
        "3. Только точное совпадение по механизму Tier 1 / feedback_exact "
        ...
        "5. Если ни одна запись не прошла проверку применимости — прямо "
...
def _build_cluster_instruction(
    ...
    instruction = (
        ...
        "markdown-заголовков:\n"
        "\n"
        "ЧТО СЛОМАЛОСЬ: 1-2 предложения. Если подходит запись базы знаний "
        "#1, начни объяснение с неё и затем добавь подтверждение из "
        "ошибки, трейса или лога.\n"
        "\n"
        "ПРИЧИНА: ровно одна категория из списка: "
        "тест / приложение / окружение / данные. "
        "Формат строки: '<категория> — <краткое обоснование>'. "
        "Если использована база знаний, обязательно назови запись базы "
        "знаний #1 или её название.\n"
        "\n"
        "КАК ИСПРАВИТЬ:\n"
    ...
    if kb_matches:
        decision_rules = [
            "Определи механизм совпадения записи базы знаний #1 по тегу "
            "tier. Если это Tier 1 / feedback_exact — запись применима "
            "безусловно, используй её как основную причину. Высокий score "
            "без этого механизма точным совпадением не считается — "
            "переходи к проверке применимости.",
            "Для всех остальных записей ответь себе на вопрос: «Совпадает "
            "ли «Пример ошибки из базы знаний» по сути с данными кластера "
            "— тот же компонент, тот же тип сбоя, те же ключевые фразы?»\n"
            "   • ДА (есть дословный или близкий фрагмент, кроме общих "
            "слов) — процитируй этот фрагмент в поле «ЧТО СЛОМАЛОСЬ» "
            "и используй запись.\n"
            "   • НЕТ — НЕ используй запись, даже если score 0.79, 0.85 "
            "или 0.95. Выведи анализ только по сообщению ошибки, "
            "трейсу и логу.",
            "При score 0.65-0.85 строгая презумпция — запись НЕ применима, "
            "пока ты не нашёл и не процитировал конкретный фрагмент из "
            "«Пример ошибки из базы знаний» в данных кластера. Высокого "
            "score по Tier 2 (построчное) или Tier 3 (TF-IDF) самого по "
            "себе НЕДОСТАТОЧНО.",
            "Если ни одна запись не прошла проверку применимости — прямо "
            "напиши «подходящих записей в базе знаний нет» и строй анализ "
            "без них. Не подгоняй.",
            "Записи базы знаний #2 и #3 — только как дополнительный "
            "контекст и только если прошли ту же проверку применимости, "
            "что и запись #1.",
        ]
    else:
        decision_rules = [
