...
def build_cluster_analysis_prompt(
    ...
    *,
    ...
    if has_symptom and has_log:
        parts.append(
            "\nПодсказка: «Сообщение об ошибке» / «Стек-трейс» — это "
            "реакция теста (симптом), «Фрагмент лога» — поведение "
            "приложения (часто первопричина). Свяжи их в анализе."
        )
...
_SYSTEM_PROMPT_CLUSTER = "\n".join(
    [
        ...
        "Если чего-то нет в ошибке, стек-трейсе, логе, шаге теста или "
        "базе знаний — не упоминай это.",
        "",
        ...
        "ошибок») и строй вывод по сообщению ошибки и трейсу; «Шаг "
        "теста», если он есть, можно использовать как вспомогательный "
        "контекст (см. пункт 9).",
        ...
        "по ошибке / трейсу / логу; «Шаг теста», если он есть, можно "
        "использовать как вспомогательный контекст (см. пункт 9).",
...
def _append_kb_section(
    ...
    if not _is_exact_kb_match(top_match):
        parts.append(
            ...
            "сути сбоя» и строй анализ по ошибке, трейсу и логу (плюс "
            "«Шаг теста», если он есть, — как вспомогательный контекст).\n"
...
def _build_cluster_instruction(
    ...
    *,
    ...
    if has_symptom and has_log:
    ...
    elif has_log and not has_symptom:
        what_broke = (
            "ЧТО СЛОМАЛОСЬ: 1-2 предложения по фрагменту лога "
            "приложения. В начале явно укажи: «сообщение об ошибке "
            "и стек-трейс отсутствуют, анализ построен по логу "
            "приложения». Затем приведи дословную цитату ключевой "
            "строки лога (исключение, ошибка БД, сообщение сервиса). "
            "Если подходит запись базы знаний #1, начни объяснение "
            "с неё и затем добавь подтверждение из лога.\n"
        )
        cause = (
            "ПРИЧИНА: ровно одна категория из списка: "
            "тест / приложение / окружение / данные. "
            "Формат строки: '<категория> — <краткое обоснование>'. "
            "Категорию определи по содержимому лога приложения. "
            "Если использована база знаний, обязательно назови запись "
            "базы знаний #1 или её название.\n"
        )
        fix = (
        ...
        what_broke = (
            ...
            "явных ошибок», поэтому анализ построен по ошибке теста "
            "(и «Шагу теста», если он есть, — как вспомогательному "
            "контексту). Если подходит запись базы знаний #1, начни "
        ...
        cause = (
            ...
            "сообщению ошибки, трейсу и, при дефиците данных, «Шагу "
            "теста» как вспомогательному контексту. "
            "Если использована база знаний, обязательно назови запись "
            "базы знаний #1 или её название.\n"
        )
        fix = (
    ...
    if has_symptom and has_log:
        ...
        decision_rules.append(
            "Сообщения об ошибке и стек-трейса нет — анализ строй "
            "по фрагменту лога приложения; «Шаг теста», если он есть, "
            "используй как вспомогательный контекст. Не выдумывай "
            "симптом со стороны теста."
        )
    else:
        decision_rules.append(
    ...
    if kb_matches:
        decision_rules.extend(
            [
                ...
                "0.85 или 0.95. Выведи анализ по сообщению ошибки, "
                "трейсу и логу (плюс «Шаг теста», если он есть, — "
                "как вспомогательный контекст).",
        ...
        decision_rules.append(
            "Базы знаний нет — опирайся на сообщение ошибки, трейс и "
            "лог; «Шаг теста», если он есть, используй как "
