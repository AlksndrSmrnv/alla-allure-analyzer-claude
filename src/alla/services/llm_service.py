...
def build_cluster_prompt(
    ...
    instruction = (
        "Напиши краткий анализ по 4 пунктам. Будь лаконичен — 2-4 предложения на пункт.\n"
        "\n"
        "1. ЧТО ПРОИЗОШЛО — перескажи суть ошибки своими словами. "
        "Называй только те классы, методы, строки и значения, "
        "которые РЕАЛЬНО есть в данных выше.\n"
        "\n"
        "2. КАТЕГОРИЯ — одна из: тест / приложение / окружение / данные. "
        "Одно предложение почему.\n"
        "\n"
        "3. ЧТО ДЕЛАТЬ — 1-3 коротких совета."
    )
    ...
    if kb_matches:
        ...
        if high_confidence:
            instruction += (
                ...
                "используй шаги решения оттуда."
            )

    instruction += (
        " Каждый совет должен опираться на конкретику из ошибки или лога. "
        "Не давай советов, которые нельзя вывести из данных выше.\n"
        "\n"
        "4. КРИТИЧНОСТЬ — critical / high / medium / low. Одно предложение.\n"
        "\n"
