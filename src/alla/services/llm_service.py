...
def build_cluster_prompt(
    ...
    *,
    ...
    if kb_matches:
        decision_rules = [
            "Шаг 1. Определи механизм совпадения записи базы знаний #1 по тегу tier. "
            "Если это Tier 1 / feedback_exact (score >= 0.999) — запись применима безусловно, "
            "используй её как основную причину.",
            "Шаг 2. Для всех остальных записей ответь себе на вопрос: «Совпадает ли "
            ...
            "Шаг 3. При score 0.65-0.85 строгая презумпция — запись НЕ применима, пока ты не "
            ...
            "Шаг 4. Если ни одна запись не прошла проверку применимости — прямо напиши "
            "«подходящих записей в базе знаний нет» и строй анализ без них. Не подгоняй.",
            "Шаг 5. Записи базы знаний #2 и #3 — только как дополнительный контекст и только "
