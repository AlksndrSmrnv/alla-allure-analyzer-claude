...
def _interpret_kb_match(match: KBMatchResult) -> str:
    """Перевести совпадение с базой знаний в текстовое описание уверенности.

    «Точное» — только для Tier 1 / feedback_exact. Высокий score без точного
    механизма матчинга (например, Tier 2 со step_path-бустом до 1.0) описывается
    как «высокое», чтобы не конфликтовать с `_is_exact_kb_match` и правилами
    промпта.
    """
...
def build_cluster_prompt(
    ...
    *,
    ...
    system_parts: list[str] = [
        ...
        "3. Только точное совпадение по механизму Tier 1 / feedback_exact можно использовать "
        "как обязательную основную причину без дополнительной проверки. "
        "Высокий score (в т.ч. 0.999+) без такого механизма точным совпадением НЕ считается.",
    ...
    if kb_matches:
        ...
        parts.append(
            ...
            "  • Механизм Tier 1 / feedback_exact (точное совпадение) → обязательная "
            "основная причина. Высокий score без этого механизма сюда НЕ попадает.\n"
            "  • score 0.65-0.999 без точного механизма → КАНДИДАТ. Принять можно ТОЛЬКО если ты процитируешь "
    ...
    if kb_matches:
        decision_rules = [
            ...
            "Если это Tier 1 / feedback_exact — запись применима безусловно, "
            "используй её как основную причину. Высокий score без этого механизма "
            "точным совпадением не считается — переходи к проверке применимости.",
