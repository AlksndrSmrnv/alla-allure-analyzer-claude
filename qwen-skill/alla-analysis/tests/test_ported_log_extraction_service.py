from alla_skill.services.log_extraction_service import _extract_error_blocks
from unittest.mock import patch
from alla_skill.services.log_extraction_service import _decode_text, _detect_content_type
from alla_skill.services.log_extraction_service import _extract_text_http_info
from alla_skill.services.log_extraction_service import (
...
from alla_skill.models.common import TestStatus as StatusEnum
from alla_skill.services.log_extraction_service import LogExtractionConfig, LogExtractionService
from .skill_test_support import make_failed_test_summary
...
class TestDetectContentType:
    ...
    def test_json_heuristic_for_text(self):
        """Содержимое JSON определяется без системной библиотеки."""
        content = b'{"RqUID": "abc"}'
        assert _detect_content_type(content) == "json"
...
class TestDetectAndExtractHttp:
    ...
    def test_ndjson_multiple_objects(self):
        content = b'{"RqUID": "r1", "error": "first"}\n{"RqUID": "r2", "statusCode": 500}\n'
        result = _detect_and_extract_http(content, "json")
        assert "r1" in result
        assert "r2" in result
...
class TestScanJsonForHttpInfo:
    ...
    def test_deeply_nested_error(self):
        obj = {
            "response": {
                ...
                "body": {"errors": [{"errorCode": "E001", "errorMessage": "Service down"}]},
            }
        }
        result = _scan_json_for_http_info(obj)
    ...
    def test_list_of_objects(self):
        obj = [{"RqUID": "id-1", "error": "first failure"}, {"RqUID": "id-2", "statusCode": 503}]
        result = _scan_json_for_http_info(obj)
...
def make_summary(test_result_id: int = 1) -> FailedTestSummary:
    return FailedTestSummary(
        test_result_id=test_result_id, name="test_foo", status=StatusEnum.FAILED
    )
...
class TestLogExtractionServiceIntegration:
    ...
    async def test_text_attachment_extracts_both_log_and_http(self):
        content = b'2026-01-01 10:00:00 [ERROR] Connection refused\n    at Service.java:42\n"RqUID": "mixed-id"\n"error": "upstream timeout"\n'
    ...
    async def test_attachment_name_with_newline_sanitized(self):
        ...
        att = AttachmentMeta(id=20, name="Отправлен запрос ->\n", type="application/json")
    ...
    async def test_log_snippet_is_truncated_when_exceeds_max_chars(self):
        ...
        big_trace = "\n".join(
            (f"    at com.example.deep.Frame{i}.call(Frame{i}.java:{i})" for i in range(500))
        )
        content = f"2026-02-09 10:23:45,123 [ERROR] BoomException\n{big_trace}".encode("utf-8")
        ...
        service = LogExtractionService(
            provider, LogExtractionConfig(concurrency=1, max_snippet_chars=500)
        )
        summary = make_summary()
        await service.enrich_with_logs([summary])
        assert summary.log_snippet is not None
...
class TestDetailsAttachmentAugmentation:
    ...
    async def test_details_accepts_content_type_when_type_is_missing(self):
        ...
        att = AttachmentMeta(id=114, name="Details", type=None, content_type="text/plain")
    ...
    async def test_multiple_details_uses_first_and_logs_skipped_rest(self, caplog):
        ...
        second_details = "Expected: <second>\nActual: <skipped>\n2026-01-01 10:00:00 [ERROR] should not become log"
        atts = [
            AttachmentMeta(id=115, name="Details", type="text/plain"),
            AttachmentMeta(id=116, name="Details.txt", type="text/plain"),
        ]
        provider = FakeAttachmentProvider(
            atts, {115: first_details.encode("utf-8"), 116: second_details.encode("utf-8")}
        )
        service = LogExtractionService(provider, LogExtractionConfig(concurrency=1))
        summary = make_failed_test_summary(status_message="AssertionError")
        caplog.set_level(logging.DEBUG, logger="alla_skill.services.log_extraction_service")
        await service.enrich_with_logs([summary])
        assert summary.status_message == f"AssertionError\n\n{first_details}"
        assert summary.log_snippet is None
    ...
    async def test_details_is_idempotent_when_already_inline(self):
        ...
        service = LogExtractionService(provider, LogExtractionConfig(concurrency=1))
        summary = make_failed_test_summary(status_message=f"AssertionError\n\n{details}")
        await service.enrich_with_logs([summary])
        assert summary.status_message == f"AssertionError\n\n{details}"
    ...
    async def test_details_and_neighbor_log_are_split_between_channels(self):
        ...
        provider = FakeAttachmentProvider(atts, {108: details.encode("utf-8"), 109: log})
        service = LogExtractionService(provider, LogExtractionConfig(concurrency=1))
        summary = make_failed_test_summary(status_message="AssertionError")
        await service.enrich_with_logs([summary])
        assert summary.status_message == f"AssertionError\n\n{details}"
    ...
    async def test_details_download_error_does_not_stop_pipeline(self):
        ...
        provider = FailingDetailsProvider(
            [details_att, log_att], {111: b"2026-01-01 10:00:00 [ERROR] downstream failed"}
        )
        service = LogExtractionService(provider, LogExtractionConfig(concurrency=1))
        summary = make_failed_test_summary(status_message="AssertionError")
        await service.enrich_with_logs([summary])
    ...
    async def test_binary_garbage_details_is_skipped(self):
        ...
        service = LogExtractionService(provider, LogExtractionConfig(concurrency=1))
        summary = make_failed_test_summary(status_message="AssertionError")
        with patch("alla_skill.services.log_extraction_service._decode_text", return_value=None):
            await service.enrich_with_logs([summary])
        assert summary.status_message == "AssertionError"
