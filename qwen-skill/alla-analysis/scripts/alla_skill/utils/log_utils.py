...
_CORRELATION_KEY_PRIORITY = {key: index for index, key in enumerate(_CANONICAL_CORRELATION_KEYS)}
    ...
    def visit(value: Any, depth: int) -> None:
        ...
        if isinstance(value, dict):
            for raw_key, raw_value in value.items():
                key = str(raw_key)
                key_lower = key.lower()
                if key_lower in _CORRELATION_KEYS_LOWER and isinstance(raw_value, (str, int)):
...
def parse_log_sections(
    ...
    *,
    ...
    for index, match in enumerate(matches):
        ...
        body_end = matches[index + 1].start() if index + 1 < len(matches) else len(log_snippet)
