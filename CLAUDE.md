...
│  (KnowledgeBaseProvider      │     │  entries.yaml (общие)  │
│   Protocol impl)             │     │  project_{id}.yaml     │
│   keyword + TF-IDF matching  │     │  (per-project KB)      │
└──────────────────────────────┘     └───────────────────────┘
                │
                ▼
         KB Match Results
...
├── CLAUDE.md                       # Этот файл
├── knowledge_base/
│   ├── entries.yaml                # Глобальная KB (общие записи для всех проектов)
│   └── project_{id}.yaml           # Per-project KB (автосоздаётся при первом анализе)
└── src/
    └── alla/
...
### Per-project KB файлы

При анализе прогона загружаются **два** источника записей:
- **Глобальный** `entries.yaml` — общие записи для всех проектов.
- **Проектный** `project_{id}.yaml` — записи, специфичные для конкретного проекта.

ID проекта извлекается из ответа `GET /api/launch/{id}` (поле `projectId`). Если проектный файл не существует, он создаётся автоматически (пустой). Оба файла загружаются в один `YamlKnowledgeBase` и используются совместно.

```
knowledge_base/
  entries.yaml          # Глобальные записи
  project_42.yaml       # Записи для проекта #42 (автосоздан)
  project_17.yaml       # Записи для проекта #17 (автосоздан)
```
...
Записи хранятся в YAML-файлах в каталоге `knowledge_base/`. Глобальные — в `entries.yaml`, проектные — в `project_{id}.yaml`. Формат одинаковый. Поле `error_example` — фрагмент ошибки из лога для TF-IDF сопоставления:
